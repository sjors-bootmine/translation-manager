<?php namespace OpenAdmin\TranslationManager;

use OpenAdmin\TranslationManager\Manager;
use Illuminate\Http\Request;
use Illuminate\Routing\Controller as BaseController;
use OpenAdmin\TranslationManager\Models\Translation;
...
{
    /** @var \OpenAdmin\TranslationManager\Manager  */
    protected $manager;
    
    /** @var \OpenAdmin\Admin\Layout\Content  */
    protected $content;

    public function __construct(Manager $manager, Content $content)
    {
    ...
    public function getIndex($group = null)
    {
        ...
        ->body(view('oa-translation-manager::index')
           ...
           ->with('editUrl', $group ? action('\OpenAdmin\TranslationManager\Controller@postEdit', [$group]) : null)
    ...
    public function postAddGroup(Request $request)
    {
        ...
        if ($group) {
            return redirect()->action('\OpenAdmin\TranslationManager\Controller@getView', $group);
        } else {
            return redirect()->back();
        }
    }
    ...
    public function postTranslateMissing(Request $request)
    {
        ...
        if ($request->has('with-translations') && $request->has('base-locale') && in_array($request->input('base-locale'), $locales) && $request->has('file') && in_array($newLocale, $locales)) {
            ...
            foreach ($base_strings as $base_string) {
                ...
                app()->call(
                    'OpenAdmin\TranslationManager\Controller@postEdit',
                    [
                        'group' => $group
                    ]
                );
